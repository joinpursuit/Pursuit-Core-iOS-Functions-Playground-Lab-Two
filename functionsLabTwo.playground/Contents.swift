...
func frequencyDictionary(of: String) -> [Character:Int]{
    var someDictionary: [Character:Int] = [:]
    var counter = 0
    
    for char in of{
        someDictionary.updateValue(0,forKey:char)
    }
    
    for char in of{
        counter = someDictionary[char] ?? 0
        someDictionary.updateValue(counter + 1,forKey:char)
    }
...
func value(inputOne:Double ,isGreaterThanAverageOf:[Double])  -> Bool {
    var sum: Double = 0
    var avg: Double = 0
    
    for i in isGreaterThanAverageOf {
        sum += i
    }
    
    avg = sum / Double(isGreaterThanAverageOf.count)
...
func secondSmallestValue(in x: [Int]) -> Int {
    ...
    var secondSmallestValue: Int = 0
    
    if x[0] > x[1]{
        smallestValue = x[1]
        secondSmallestValue = x[0]
    } else {
        smallestValue = x[0]
        secondSmallestValue = x[1]
    }
    
    for index in 2..<x.count {
        if x[index] < smallestValue{
            secondSmallestValue = smallestValue
            smallestValue = x[index]
        } else if x[index] > smallestValue && x[index] < secondSmallestValue{
            secondSmallestValue = x[index]
        } else {
