...
func average(of grades: [Double]) -> Double {
    var sum = 0.0
    for grade in grades{
        sum += grade
    }
    var average: Double
    average = sum/Double(grades.count) // Doubles cant be divided by an Int, I had to cast it
    return average
}
...
func frequencyDictionary(of str: String) -> [String: Int] {
    
    
    let stringAsSet = Set(str)
    //print(stringAsSet)
...
frequencyDictionary(of: "Helllo")

let testCasesTwo: [(String, [String: Int])] = [
...
func fizzBuzz(upto number :Int) -> [String] {
   ...
   /*
    ...
    for num in numArray {
        if num % 3 == 0 && num % 5 == 0 {
            stringArray[num] = "FizzBuzz"
        } else if num % 3 == 0 {
            stringArray[num] = "Fizz"
        } else if num % 5 == 0 {
            stringArray[num] = "Buzz"
        }
...
func secondSmallestValue(in numArray: [Int]) -> Int {
    // find the smallest num and then remove and then assign the smallest num of that array will now be the smallest
    // OR
    //use .sorted and then the second smallest is the second index.
    
    let sortedIntArray = numArray.sorted()
    let secondSmallestValue = sortedIntArray[1]
