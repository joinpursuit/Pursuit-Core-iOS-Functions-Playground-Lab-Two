...
// Your function here
func averageOf(arrDouble : [Double]) -> Double    {
    var sum = 0.0
    var average = 0.0
    for num in arrDouble    {
        sum += num
    }
    average = sum/Double((arrDouble.count))
    return average
}
...
for (input, expectedOutput) in testCasesOne {
    let output = averageOf(arrDouble: input)
...
print(frequencyDictionary(of: "hoot"))
...
// Your function here
func value(number : Double, isGreaterThanAverageOf: [Double]) -> Bool {
    var sum = 0.0
    var average = 0.0
    var biggerNumber : Bool
    for num in 0...isGreaterThanAverageOf.count - 1   {
        sum += isGreaterThanAverageOf[num]
    }
...
for (inputOne, inputTwo, expectedOutput) in testCasesFour {
    let output = value(number: inputOne, isGreaterThanAverageOf: inputTwo)
...
// Your function here
func secondSmallest(arrayInts : [Int]) -> Int  {
    //arrayInts.sorted()
    let secondSmall = arrayInts.sorted()
    return secondSmall[1]
}
...
for (input, expectedOutput) in testCasesFive {
    let output = secondSmallest(arrayInts: input)
    assert(output == expectedOutput, "Was expecting \(expectedOutput) for input \(input), but got \(output)")
}
