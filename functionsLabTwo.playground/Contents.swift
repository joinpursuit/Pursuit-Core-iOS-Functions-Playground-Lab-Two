...
// Your function here
func fizzBuzz(upto num: Int) -> [String] {
    var fizzArr = [String]()
    for number in 1...num {
        if number % 15 == 0 {
            fizzArr.append("FizzBuzz")
        } else if number % 5 == 0 {
            fizzArr.append("Buzz")
        } else if number % 3 == 0 {
            fizzArr.append("Fizz")
        } else {
            fizzArr.append(String(number))
        }
    }
    return fizzArr
}
...
// Your function here
func value(_ num: Double, isGreaterThanAverageOf arr: [Double]) -> Bool {
    var sum = 0.0
    for doub in arr {
        sum += doub
    }
    return num > (sum / Double(arr.count)) ? true : false
}
...
// Your function here
func secondSmallestValue(in arr: [Int]) -> Int {
